...
CREATE OR REPLACE PACKAGE BODY cities_shortest_way IS
    ...
    FUNCTION min_way (
        ...
        new_visit  IN row_type,
        v_start IN NUMBER,
        v_beginning IN NUMBER
    )
    RETURN NUMBER AS
    min_dist NUMBER;
    min_vers NUMBER;
    ...
    min_dist:=10000;
     ...
     IF (min_dist>v_matrix(v_start)(i)
     AND NOT new_visit.EXISTS(i) AND v_matrix(v_start)(i)<>-1
     AND i<>v_beginning)  THEN
     min_dist:=v_matrix(v_start)(i);
     min_vers:=i;
     END IF;
     END LOOP;
     RETURN min_vers;
    ...
    BEGIN
        ...
        new_visit(v_start) := 0;
        IF (new_visit.count = v_matrix.count) THEN
            new_path(v_count+1):=v_beginning;
            v_result.distance := v_distance+v_matrix(v_start)(v_beginning);
            v_result.path := new_path;
        END IF;
        
        IF (new_visit.count <> v_matrix.count) THEN
            vers := v_matrix(v_start)(min_way(v_matrix,new_visit,v_start,v_beginning));
            IF ( NOT (vers = -1 )) THEN
                visit_vers(v_result, new_path, v_matrix, min_way(v_matrix,new_visit,v_start,v_beginning), v_finish,
                          new_visit, v_distance + vers, v_count + 1,v_beginning);
            END IF;
    END IF;
